...
现有pci总线,然后才有所谓的pci插槽.qemu中通过 `i440fx_init` 函数来初始化PCI总线.大致的调用关系是 `pc_init1 -> i440fx_init`.
...
在qemu0.4.4里面,我们来看一下ne2000是如何来实现映射的.
...
通过设置PCI接口的相关寄存器,我们就可以轻易改变ne2000的访问地址.
...
实际上 `pci_dev->config_write` 对应的函数实际上是 `pci_default_write_config`,这是一个相对复杂的函数:
